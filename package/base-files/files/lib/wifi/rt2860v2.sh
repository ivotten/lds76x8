...
Key3Str=
Key4Str=
WscManufacturer=GuangZhou Yuandianhe .inc
WscModelName=MediaTek Wireless Router
...
#					iwpriv $ifname set ApCliBssid=$bssid
			...
			case "$encryption" in
				none)
					...
					ap_client ra0 $ifname $ssid  "" &
					;;
				WEP|wep|wep-open)
					echo "WEP" >>/tmp/wifi_encryption_${ifname}.dat
					ap_client ra0 $ifname $ssid  $key &
					;;
...
#判断系统是否存在rt2860v2_ap，不存在则退出
	...
	[ -d mt76x3_ap ] || [ -d mt7628 ] || [ -d rt2860v2_ap ] return
	#检查并创建WiFi驱动配置链接
	[ -f /etc/Wireless/RT2860/RT2860AP.dat ] || {
	mkdir -p /etc/Wireless/RT2860/ 2>/dev/null
	touch /tmp/RT2860.dat
...
#检测系统存在多少个wifi接口
	...
	uci -q batch <<-EOF
		set wireless.ra${i}=wifi-device
		set wireless.ra${i}.type=rt2860v2
		set wireless.ra${i}.mode=7
		set wireless.ra${i}.channel=auto
		...
		set wireless.sta.ssid=YourSSID
                set wireless.sta.encryption=psk2
                set wireless.sta.key=YourPWD
EOF
